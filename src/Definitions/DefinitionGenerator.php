...
namespace Kai\L5Swagger\Definitions;
 ...
 *
 * @package Kai\L5Swagger\Definitions
 */
class DefinitionGenerator
{
    /**
            ...
            if ($obj instanceof Model) { //check to make sure it is a model
                ...
                foreach ($appends->getValue($obj) as $item) {
                    ...
                    if (!$reflection->hasMethod($methodeName)) {
                        Log::warning("[Kai\L5Swagger] Method $model::$methodeName not found while parsing '$item' attribute");
                        continue;
                    }
